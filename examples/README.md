...
1. Check connectivity
    ```bash
    docker exec -ti firebolt-core firebolt -c "SELECT 42;"
    ```

2. See example on how to list objects in public [S3](./list_objects_s3.sql) & [GCS](./list_objects_gcs.sql) bucket.
    ```bash
    docker exec -i firebolt-core firebolt -f - < ./list_objects_s3.sql
    ```
    ```bash
    docker exec -i firebolt-core firebolt -f - < ./list_objects_gcs.sql
    ```

3. [See example on how to](./read_parquet.sql) read **Parquet** data.
    ```bash
    docker exec -i firebolt-core firebolt -f - < ./read_parquet.sql
    ```

4. [See example on how to](./read_iceberg.sql) read **Iceberg** data.
    ```bash
    docker exec -i firebolt-core firebolt -f - < ./read_iceberg.sql
    ```

5. [See example on how to](./create_table.sql) create a table from an external file.
    ```bash
    docker exec -i firebolt-core firebolt -f - < ./create_table.sql
    ```
...
6. See example on how to create an [aggregating index](https://docs.firebolt.io/sql_reference/commands/data-definition/create-aggregating-index.html) to improve performance.
    ...
    ```bash
    docker exec -i firebolt-core firebolt -f - < ./inflate_lineitem.sql
    ```

    6.2 Run a simple aggregation query
    ```bash
    docker exec -i firebolt-core firebolt -f - < ./simple_aggregation.sql
    ```

    6.3 Create the aggregating index
    ```bash
    docker exec -i firebolt-core firebolt -f - < ./aggregating_index.sql
    ```
